 ...
 */
 ...
 exports.basicAuth = function (username, password) {
   return function (req, res, next) {
     ...
     var isForPublicAsset = req.url.indexOf('/public') === 0
...
var sanitiseStoredData = function(input) {

  // storing this for single source of truth
  var uncheckedString = '_unchecked';

  // reference the input
  var val = input;

  // delete single unchecked checkboxes
  if (val === uncheckedString || val === [uncheckedString]) {
    return null // explicit about it being not found
  }

  // is this an array (checkboxes produce these)
  if (Array.isArray(val)) {
      // remove all instances of _unchecked from the array
      for(var i = val.length-1; i--;){
      	if (val[i] === uncheckedString) val.splice(i, 1);
      }
      // if the array is now only 1 item, set it directly as the value
      // but it will also need to be sanitised
      if(val.length == 1) {
        val = sanitiseStoredData(val[0]);
      }
  }

  // is this an object? You can do this is in node by naming inputs with
  // object[key] notation for example:
  //
  // name="employeeStatus[employed]" for checkbox 1
  // name="employeeStatus[selfEmployed]" for checkbox 2
  //
  // this would give you an object like this:
  // employeeStatus: { employed: 'true', selfEmployed: 'true' }
  //
  // We need to handle those and delete unchecked values
  if (typeof val === 'object') {

    // loop over the keys of the object
    Object.keys(val).forEach(function(key) {
      // if the value of the object property/key is === uncheckedString
      // then deletes it
      if (val[key] === uncheckedString) {
        delete val[key];
      } else {
        // if value is also an object the rescurivsely do the same
        if(typeof val[key] === 'object') {
          val[key] = sanitiseStoredData(val[key]);

        // else remove uncheckedString from arrays
        } else if (Array.isArray(val[key])) {

          // loops the array removing items matching the uncheckedString
          for(var i = val[key].length-1; i--;){
          	if (val[key][i] === uncheckedString) val[key].splice(i, 1);
          }
          // if the array is now just a single item, rerun it through this same
          // sanitise function and set at the direct value of the key
          if (val[key].length == 1) {
            val[key] = sanitiseStoredData(val[key][0])
          }
        }
    ...
    // if we now have an empty object, return nothing
...
var storeData = function (input, store) {
  for (var i in input) {
    ...
    // santises the value
    var val = sanitiseStoredData(input[i])
