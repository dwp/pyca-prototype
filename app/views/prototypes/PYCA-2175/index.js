...
router.all('/accessibility', (req, res) => {
    res.render(`${__dirname}/views/accessibility`)
});
...
router.all('/:type/questions/british-passport', (req, res) => {
    ...
    if (submitted.ukNational === 'yes') {
        return res.redirect('./british-national-overseas-visa')
    }
...
router.all('/:type/questions/nationality', (req, res) => {
    ...
    if (type === 'claimant' && submitted.nationality) {
        ...
        if (claimant.nationality === 'country:GB' || claimant.nationality === 'territory:HK') {
            if(saved.ukNational === 'no' && saved.refugee === 'no') {
                return res.redirect('./british-national-overseas-visa')
            }
...
/**
 * Question: Does the claimant have a British National (Overseas) visa?
 */
router.all('/:type/questions/british-national-overseas-visa', (req, res) => {
    const type = req.params.type
    const submitted = req.body[type]
    const saved = req.session.data[type]
    console.log(saved.nationality)
    // UK national
    if (submitted.ukNationalOverseasVisa === 'yes') {
        //GOTO NEW FAST TRACK PAGE
       return res.redirect('../../outcome/END034')
    }

    // Non-UK national
    if (submitted.ukNationalOverseasVisa === 'no') {
        if(saved.ukNational === 'yes') {
            return res.redirect('./british-passport-today')
        }
        if(saved.ukNational === 'no') {
            if( saved.nationality === 'country:GB') {
                return res.redirect('../../outcome/END300')
            }
            if( saved.nationality === 'territory:HK') {
                return res.redirect('./residence-pre-settled-status')
            }
        }
        return res.redirect('./refugee')
    ...
    res.render(`${__dirname}/views/questions/british-national-overseas-visa`)
})
...
router.all('/:type/questions/residence-permit-type', (req, res) => {
    ...
    // Residence or EU Scheme pre-settlement
    if (submitted.brpType === 'residence' || submitted.brpType === 'eu scheme pre-settlement') {
        return res.redirect('./residence-permit-sub-type')
    }

    // EU Scheme settlement
    if (submitted.brpType === 'eu scheme settlement') {
        return res.redirect('./residence-out-of-uk')
    }
...
/**
 * Question: In the last 2 years, have they been out of the UK for 4 weeks or more in a single trip?
 */
router.all('/:type/questions/residence-out-of-uk', (req, res) => {
    const type = req.params.type
    const submitted = req.body[type]
    ...
    res.render(`${__dirname}/views/questions/residence-out-of-uk`);
});

/**
 * Question: When did they return to the UK from their most recent trip of 4 weeks or more?
 */
router.all('/:type/questions/residence-return-to-uk', (req, res) => {
    const type = req.params.type
    const submitted = req.body[type]
 ...
 */
router.all('/:type/questions/residence-currently-working', (req, res) => {
    const type = req.params.type
    const submitted = req.body[type]
    ...
    res.render(`${__dirname}/views/questions/residence-currently-working`);
});
...
router.all('/:type/questions/employment-status', (req, res) => {
    ...
    if ((submitted.employmentStatus || []).includes('employed') && (submitted.employmentStatus || []).includes('selfEmployed')) {
